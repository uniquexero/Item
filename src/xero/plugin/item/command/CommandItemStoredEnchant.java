...
public class CommandItemStoredEnchant extends ItemCommand
{

	public CommandItemStoredEnchant()
	{
		super("<type> <level>", "마법부여된 책의 인챈트를 변경합니다.", Permissions.ENCHANTED_BOOK, 2);
	}
	...
	public boolean onCommand(Player sender, Command command, String label, String componentLabel, ArgumentList args, XeroItemStack item)
	{
		...
		if (level > 0)
		{
			...
			NBTList list = tag.getList("StoredEnchantments");
			
			if (list == null)
			{
				list = NBTList.newInstance();
				tag.setList("StoredEnchantments", list);
			}
						...
						if (c.getShort("id") == id)
						{
							if (size > 1)
							{
								...
								for (int j = 0; j < size; j++)
								{
									...
									tag.setList("StoredEnchantments", other);
								}
							}
							else
							{
